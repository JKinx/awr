...
def parse_args(args):
    ...
    parser.add_argument("--train-data", dest="train_data", default="")
...
def main(args):
    ...
    train_data = pickle.load(open(arg_parser.train_data, "rb"))

    agent.clear_buffer()
    for path in tqdm(train_data):
        agent.add_to_buffer(path)

    if (arg_parser.train):
        agent.op_train(max_iter=arg_parser.max_iter,
